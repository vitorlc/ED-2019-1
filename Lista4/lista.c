...
NoInt *no_int_obtem(NoInt *cabeca, int posicao) {
    int posicao_corrente = 0;
    NoInt *noCorrente = cabeca;
    if(posicao == 0) {
        return cabeca;
    }
    for(posicao_corrente = 0; posicao_corrente < posicao && noCorrente->proximo != NULL; posicao_corrente++){
        noCorrente = noCorrente->proximo;
    }
    if(posicao_corrente == posicao) {
        return noCorrente;
    } else {
        return NULL;
    }
...
NoInt *no_int_inverte(NoInt *cabeca) {
       NoInt *noAnterior = NULL;
       NoInt *noCorrente = cabeca;
       NoInt *noProximo  = NULL;
       while(noCorrente != NULL) {
               noProximo = noCorrente->proximo;
               noCorrente->proximo = noAnterior;
               noAnterior = noCorrente;
               noCorrente = noProximo;
       }
       return noAnterior;
}
int
